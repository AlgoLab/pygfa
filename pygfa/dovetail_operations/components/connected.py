...
def _plain_bfs_dovetails_with_edges(gfa_, source, keys=False):
    ...
    a BFS operation, giving BFS edges also.
    ...
    while len(queue):
        ...
        for from_, to_, key in gfa_.dovetails_neighbors_iter(from_node, keys=True):
            if to_ not in seen:
...
def dovetails_nodes_connected_components(gfa_, with_sequence=True):
    ...
    :param with_sequence: If set start the computation considering
        only nodes where the 'sequence' property is present. Consider
        every node in the graph otherwise.
    """
    seen = set()
