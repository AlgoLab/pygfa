"""
Iterators used by GFA graph.
This iterators work considering only edges
representing dovetails overlaps.
"""

# Note that "self" will be referred to GFA class, keep this in mind
...
class DovetailIterator:
    def dovetails_iter(self, nbunch=None, keys=False, data=False):
        ...
        :notes:
            ...
            edges already seen, so edge (u,v) is in
        ...
        for from_node, to_node, key, edge_ in self.edges_iter(nbunch, keys=True, data=True):
            if "is_dovetail" in edge_ and edge_["is_dovetail"]:
                if data is True:
                    yield (from_node, to_node, key, edge_) if keys else (from_node, to_node, edge_)
                else:
                    yield (from_node, to_node, key) if keys else (from_node, to_node)
            ...
            def bunch_iter(nlist, adj):
                ...
                # It's a possible dead code?
                    ...
                    # capture error for unhashable node.
                        ...
                        raise NetworkXError("Node %s in the sequence nbunch is not a valid node." % n)
                    else:
                        raise
    ...
    def dovetails_neighbors_iter(self, nbunch=None, keys=False, data=False):
        ...
        :notes:
            ...
            If only to_node in neighborhood are needed, consider using
            `dovetails_neighbors'.
        """
    ...
    def right_end_iter(self, nbunch, keys=False, data=False):
        ...
        for nid in nids:
            for from_node, to_node, key, edge_ in self.dovetails_neighbors_iter(nid, keys=True, data=True):
                if nid == edge_["from_node"] and edge_["from_segment_end"] == "R":
                    if data is True:
                        yield (from_node, to_node, key, edge_) if keys else (from_node, to_node, edge_)
                    else:
                        yield (from_node, to_node, key) if keys else (from_node, to_node)
                if nid == edge_["to_node"] and edge_["to_segment_end"] == "R":
                    if data is True:
                        yield (from_node, to_node, key, edge_) if keys else (from_node, to_node, edge_)
                    else:
                        yield (from_node, to_node, key) if keys else (from_node, to_node)
    ...
    def left_end_iter(self, nbunch=None, keys=False, data=False):
        ...
        for nid in nids:
            for from_node, to_node, key, edge_ in self.dovetails_neighbors_iter(nid, keys=True, data=True):
                if nid == edge_["from_node"] and edge_["from_segment_end"] == "L":
                    if data is True:
                        yield (from_node, to_node, key, edge_) if keys else (from_node, to_node, edge_)
                    else:
                        yield (from_node, to_node, key) if keys else (from_node, to_node)

                if nid == edge_["to_node"] and edge_["to_segment_end"] == "L":
                    if data is True:
                        yield (from_node, to_node, key, edge_) if keys else (from_node, to_node, edge_)
                    else:
                        yield (from_node, to_node, key) if keys else (from_node, to_node)
    ...
    def dovetails_linear_path_traverse_nodes_iter(self, source):
        """Traverse all nodes adjacent to the source node where
        right degree and left degree of each node
        is 1.
        ...
        while nextlevel:
            ...
            nextlevel = set()
            for v in thislevel:
                if v not in seen and self.right_degree(v) <= 1 and self.left_degree(v) <= 1:
                    yield v
                    seen.add(v)
    ...
    def dovetails_linear_path_traverse_edges_iter(self, source, keys=False):
        ...
        """
        is_linear = lambda gfa_, node_: gfa_.right_degree(node_) <= 1 and gfa_.left_degree(node_) <= 1
        ...
        while len(queue):
            from_node = queue.pop()
            for from_, to_, key in self.dovetails_neighbors_iter(from_node, keys=True):
                if to_ not in seen:
                    if is_linear(self, to_):
                        yield (from_, to_, key) if keys else (from_, to_)
                        seen.add(to_)
                        queue.append(to_)
