...
def real_overlap(from_sequence: str, to_sequence: str) -> int:
    ...
    while start_pos_to != 0 or not start_pos_from + size == len(from_sequence):
        start_pos_from, start_pos_to, size = sequence_overlap.find_longest_match(
            start, len(from_sequence), 0, len(to_sequence)
        )
...
def consistency(node, sequence, orientation, overlap):
    ...
    if not size_overlap == overlap:
        GRAPH_LOGGER.debug(
            'Edge between node %s and %s have \
        	no consistency between CIGAR overlap end "real" overlap',
            from_id,
            to_id,
        )
        return False
...
def check_overlap(gfa_, path, external_file):
            ...
            for eid in data_edges[node1][node2]:
                if not eid_dict.get(eid):
                    ...
                    if not node_dict[from_id] == "*" and not node_dict[to_id] == "*":
                        check = consistency(
                            (from_id, to_id),
                            (node_dict[from_id], node_dict[to_id]),
                            (from_orn, to_orn),
                            overlap,
                        )
                        if check:
                        ...
                        GRAPH_LOGGER.debug(
                            "Can't check overlap consistency between \
                            node %s and %s",
                            from_id,
                            to_id,
                        )
    ...
    GRAPH_LOGGER.debug(
        "%s edge overlap are consistency of total amount of %s",
        count_consistency,
