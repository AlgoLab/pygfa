        ...
        for edge_id in data_update_edges[node]:
            if data_update_edges[node][edge_id]["from_node"] == remove_id:
                ...
                if not parse_from_orn:
                    ...
                    if orn == "-+":
                        ...
                        parse_from_positions = (
                            str(remove_slen - int(parse_from_positions[1])),
                            str(remove_slen - int(parse_from_positions[0])),
                        )
                    elif orn == "+-":
                        parse_to_orn = reverse_strand(parse_to_orn)
                        parse_from_positions = (
                            str(keep_slen - int(parse_from_positions[1])),
                            str(keep_slen - int(parse_from_positions[0])),
                        )
                    elif orn == "++":
                        parse_from_positions = (
                            str(keep_slen - remove_slen + int(parse_from_positions[0])),
            ...
            if parse_from_orn:
                ...
                parse_new_edge = (
                    ...
                    + parse_to_orn
                    + "\t"
                    + parse_from_positions[0]
                    + "\t"
                    + parse_from_positions[1]
                    + "\t"
                    + parse_to_positions[0]
                    + "\t"
                    + parse_to_positions[1]
                    + "\t"
                    + parse_overlap[0]
                )
...
def compression_graph_by_nodes(gfa_):
    ...
    GRAPH_LOGGER.debug(
        "%s edges has been compacted of total amount of %s",
        count_edge_compacted,
        len(eid_dict) - edge_no_text,
    )
...
def compression_graph_by_edges(gfa_):
        ...
        for node2 in data_edges[node1]:
            for eid in data_edges[node1][node2]:
                if not eid_dict.get(eid):
                    ...
                    from_node = (
                        data_edges[node1][node2][eid]["from_node"],
                    ...
                    to_node = (
                        data_edges[node1][node2][eid]["to_node"],
    ...
    for eid in eid_dict:
        ...
        if gfa_.nodes(identifier=from_node[0]) and gfa_.nodes(identifier=to_node[0]):
            if (
                from_node[1]
                and from_dict.get(tuple_to_string(from_node)) == 1
                and to_dict.get(tuple_to_string(to_node)) == 1
            ):
