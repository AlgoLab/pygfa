#!/usr/bin/env python3
"""Round-trip tests: GFA -> BGFA -> GFA structural equality checks.

Tests that converting a GFA to BGFA and back produces a structurally
identical graph. Both in-memory GFA constructions and file-based tests
(on files tagged with `# test: bgfa_roundtrip`) are supported.
"""
...
def _assert_structural_equality(g, h):
    """Assert that two GFA objects are structurally identical."""
    assert sorted(g.nodes()) == sorted(h.nodes()), "Segment names mismatch"
...
def pytest_generate_tests(metafunc):
    if "gfa_path" in metafunc.fixturenames and metafunc.cls is TestFileRoundtrip:
...
class TestStrictRoundtrip:
    """Tests that structural content is preserved after GFA->BGFA->GFA."""
...
# ---------------------------------------------------------------------------
# File-based structural round-trip tests
# ---------------------------------------------------------------------------


class TestFileRoundtrip:
    """Compare graph components for GFA files tagged with # test: bgfa_roundtrip."""
