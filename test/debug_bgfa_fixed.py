...
def main():
    ...
    print("Original node IDs:", g_node_ids)
    print("Original node IDs (as ints, sorted):", sorted([int(nid) for nid in g_node_ids]))
    ...
    try:
        ...
        h_node_ids = list(h.nodes())
        print("Round-trip node IDs:", h_node_ids)
        print("Round-trip node IDs (as ints, sorted):", sorted([int(nid) for nid in h_node_ids]))
        ...
        if len(g_nodes) != len(h_nodes):
            ...
            for nid in sorted(list(h_nodes), key=lambda x: int(x)):
                if nid in g_nodes:
                    g_node = g.node(nid)
                    h_node = h.node(nid)
