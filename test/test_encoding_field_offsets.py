...
- section_id (1 byte)
- record_num (2 bytes)
- segment_names_enc (2 bytes)  <- at offset +3
- clen_names (8 bytes)
- ulen_names (8 bytes)
- sequences_enc (2 bytes)    <- at offset +21
- clen_seq (8 bytes)
- ulen_seq (8 bytes)
- payload
...
class TestEncodingFieldOffsets(unittest.TestCase):
    ...
    def test_segment_names_enc_stored_at_correct_offset(self):
        """Verify segment_names_enc is stored at the correct position in BGFA file."""
        g = GFA()
        g.add_node(node_module.Node("node1", "ACGT"))

        # Use explicit encoding values to verify positions
        segment_names_enc = make_compression_code(IntegerEncoding.VARINT, StringEncoding.BROTLI)  # 0x010d
        sequences_enc = make_compression_code(IntegerEncoding.VARINT, StringEncoding.NONE)  # 0x0100

        writer = bgfa.BGFAWriter(g, 100, {"segment_names_enc": segment_names_enc, "sequences_enc": sequences_enc})
        data = writer.to_bgfa()
        ...
        self.assertEqual(
            comp_names,
            segment_names_enc,
            f"segment_names_enc mismatch: got 0x{comp_names:04x}, expected 0x{segment_names_enc:04x}",
        )
        self.assertEqual(
            comp_seq, sequences_enc, f"sequences_enc mismatch: got 0x{comp_seq:04x}, expected 0x{sequences_enc:04x}"
        )
    ...
    def test_roundtrip_with_string_encodings(self):
        ...
        for name, encoding in test_encodings:
            with self.subTest(encoding=name):
                ...
                try:
                    # Write
                    writer = bgfa.BGFAWriter(g, 100, {"segment_names_enc": encoding})
        ...
        for encoding_str, expected in test_cases:
            with self.subTest(encoding=encoding_str):
                ...
                self.assertEqual(
                    actual,
                    expected,
                    f"parse_compression_strategy('{encoding_str}') = 0x{actual:04x}, expected 0x{expected:04x}",
                )
